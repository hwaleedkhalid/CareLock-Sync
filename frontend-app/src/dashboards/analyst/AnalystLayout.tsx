/** dashboards/analyst/AnalystLayout.tsx — Dark glassmorphism, matches AdminLayout. */
import React, { useState, useEffect } from 'react';
import { NavLink, Outlet, useNavigate } from 'react-router-dom';
import { ChartColumn, LayoutDashboard, BarChart3, BookOpen, MessageSquare, LogOut, Menu, Settings, X, Zap } from 'lucide-react';
import { useAuth } from '../../shared/context/AuthContext';
import ThemeToggle from '../../shared/ui/ThemeToggle';

const NAV = [
  { title: 'Overview', items: [
    { to: '/analyst/dashboard',    label: 'Dashboard',    icon: <LayoutDashboard style={{ width: 15, height: 15 }} /> },
    { to: '/analyst/data-quality', label: 'Data Quality', icon: <BarChart3       style={{ width: 15, height: 15 }} /> },
  ]},
  { title: 'Research', items: [
    { to: '/analyst/research', label: 'Research', icon: <BookOpen style={{ width: 15, height: 15 }} /> },
  ]},
  { title: 'AI Tools', items: [
    { to: '/analyst/chat', label: 'AI Query Chat', icon: <MessageSquare style={{ width: 15, height: 15 }} />, badge: 'RAG' },
  ]},
];

const AC = { grad: 'linear-gradient(135deg,#7c3aed,#a855f7)', glow: 'rgba(139,92,246,0.40)', color: '#c084fc', pillBg: 'rgba(139,92,246,0.12)', pillBorder: 'rgba(139,92,246,0.22)' };
  ...
  <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
    <div style={{ padding: collapsed ? '1.25rem 0' : '1.25rem 1rem', borderBottom: '1px solid var(--ds-sidebar-border)', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', gap: '0.625rem', flexShrink: 0 }}>
      <div style={{ width: 34, height: 34, borderRadius: '0.625rem', background: AC.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: `0 4px 12px ${AC.glow}` }}>
        <ChartColumn style={{ width: 16, height: 16, color: '#fff' }} />
      </div>
      {!collapsed && <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: '0.6875rem', fontWeight: 600, color: AC.color }}>Analyst Portal</div>
      </div>}
    </div>
    {!collapsed && user && (
      <div style={{ margin: '0.75rem 0.625rem 0', padding: '0.75rem', background: AC.pillBg, border: `1px solid ${AC.pillBorder}`, borderRadius: '0.875rem', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: '0.6875rem', color: AC.color, fontWeight: 600 }}>Data Analyst</div>
          </div>
        </div>
      </div>
    )}
    ...
    <div style={{ padding: '0.625rem', borderTop: '1px solid var(--ds-sidebar-border)', display: 'flex', flexDirection: 'column', gap: '1px', flexShrink: 0 }}>
      <NavLink to="/analyst/settings" title={collapsed ? 'Settings' : undefined} className={({ isActive }) => `ds-nav-link${isActive ? ' active' : ''}`} style={collapsed ? { justifyContent: 'center', padding: '0.5rem 0' } : undefined}>
...
const AnalystLayout: React.FC = () => {
  ...
  const initials = user?.display_name?.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase() ?? 'AN';
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        <header className="ds-navbar" style={{ height: 56, display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0 1.25rem', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
            <Zap style={{ width: 14, height: 14, color: AC.color }} />
            <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--ds-text-secondary)' }}>Analytics Portal</span>
          </div>
          <div style={{ flex: 1 }} />
          <ThemeToggle />
